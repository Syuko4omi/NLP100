...
#v2はv1と比較して、低頻度語彙を扱わないようにして次元数を減らした（約70%減らした）。
import random
import torch
...
input_oov_idx = input_lang.num_of_words+1 #43219
output_oov_idx = output_lang.num_of_words+1 #48402
input_padding_idx = input_lang.num_of_words+2
output_padding_idx = output_lang.num_of_words+2

def indexesFromSentence(lang, sentence):
    temp_id_list = []
    for word in sentence.split():
        if word not in lang.word2id:
            temp_id_list.append(lang.num_of_words+1)
        else:
...
class LSTMEncoder(nn.Module):
    ...
    def forward(self, raw_input):
        ...
        return output, h_n # output -> (batch_size, seq_len, hidden_size),     h_n(final hidden state) -> (batch_size, 1, hidden_size)
...
class Attention(nn.Module):
    ...
    def forward(self, encoder_outputs, decoder_hidden_state, attention_mask):
        ...
        attention_score = self.scoring_function(decoder_hidden_state, encoder_outputs) #(batch_size, seq_len, 1)
...
class OneStepDecoder(nn.Module):
    ...
    def forward(self, predicted_token, decoder_hidden_state, encoder_hidden_states, decoder_cell_state, attention_mask):
        ...
        distributional_prob = self.softmax(self.fc1(torch.cat([output.squeeze(1), context_vector.squeeze(1), embedded_input.squeeze(1)], dim=1)))
        return distributional_prob, next_hidden_state, next_cell_state #predicted token -> (batch_size, vocab_size)
...
class LSTMDecoderwithAttention(nn.Module):
    ...
    def forward(self, target, encoder_hidden_states, decoder_hidden_state, attention_mask, teacher_forcing_ratio):
        ...
        teacher_force = (random.random() < teacher_forcing_ratio) #パターン1
...
train(train_tensors, 10, BATCH_SIZE) #batch size = 64, max_len = 40でも大丈夫
