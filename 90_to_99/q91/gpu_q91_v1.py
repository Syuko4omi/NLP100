...
def read_data(src_lang, dst_lang, data_type): #ここではまだlangの中身を作らない
    with open(f"kyoto-{data_type}.{src_lang}", "r") as fr_1:
        lines_1 = fr_1.readlines()
        with open(f"kyoto-{data_type}.{dst_lang}", "r") as fr_2:
...
def tensorFromSentence(lang, sentence, padding_length):
    ...
    while len(indexes) < 20: #+1はeosの分
        indexes.append(padding_idx)
    return torch.tensor(indexes[:20], dtype=torch.long).view(-1, 1)
...
train_tensors, batch_num = tensorsFromPair(pairs, batch_size = 32) #[(日本語のバッチ1, 英語のバッチ1), (日本語のバッチ2, 英語のバッチ2),...] バッチの形は(バッチサイズ*テンソルの長さ*1)
...
class LSTMDecoderwithAttention(nn.Module):
    ...
    def forward(self, target, encoder_hidden_states, decoder_hidden_state, attention_mask, teacher_forcing_ratio=0.8):
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, raw_input, target, attention_mask, teacher_forcing_ratio=0.8):
...
def train(train_tensors, epoch_num, batch_size):
    ...
    masks = [train_tensors[i][0].eq(padding_idx) for i in range(batch_num)]
    ...
    for epoch in range(1, epoch_num+1):
        ...
        for id in batch_idx_list:
            ...
            optimizer.zero_grad()
            #model_output = model(source_data, target_data, source_data.eq(padding_idx), teacher_forcing_ratio=0.8) #batch_size, target_len, vocab_size
            model_output = model(source_data, target_data, masks[id], teacher_forcing_ratio=0.8)
