...
def tensorsFromPair(batch_size):
    ...
    for batch in range(batch_num):
        ...
        input_l = torch.tensor(input_l).unsqueeze(2)
        target_l = torch.tensor(target_l).unsqueeze(2)
        temp_l.append([input_l, target_l])
...
def train(train_tensors, epoch_num, batch_size):
    ...
    for epoch in range(1, epoch_num+1):
        ...
        if epoch in checkpoint_epochs:
            ...
            torch.save(checkpoint, f'./drive/MyDrive/nlp100_98/trained_model_e{epoch}.pth')
