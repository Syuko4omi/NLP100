...
#checkpoint_file = '../trained_model/q98_model/trained_model_e10.pth'
...
for pair in test_pairs[1:]: #最初のデータは除く
    ...
    if bleu_score.sentence_bleu(target, predicted_words, smoothing_function=bleu_score.SmoothingFunction().method1) > 0.3:
        print(bleu_score.sentence_bleu(target, predicted_words, smoothing_function=bleu_score.SmoothingFunction().method1))
        print("日本語:", "".join(pair[0]))
        print("answer:", "".join(pair[1]))
        print("predicted:", en_model.decode(predicted_words))
