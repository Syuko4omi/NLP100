...
with open("train.txt", "r") as f_r_1:
    ...
    for i in range(len(L)):
        ...
        train_converted_titles.append([temp, int(train_labels[i])])
train_converted_titles.sort(key = lambda x: len(x)) #タイトルの長さでソート（同じバッチ中にあるタイトルは長さが近くなって、パディングが少なくなるように）
train_labels = torch.tensor([train_converted_titles[i][1] for i in range(len(train_converted_titles))])
train_converted_titles = [train_converted_titles[i][0] for i in range(len(train_converted_titles))]
...
class MyRNN(nn.Module):
    def __init__(self, activation_function):
        ...
        self.rnn_1 = nn.RNN(input_size = embedding_dimension, hidden_size = hidden_layer_dimension, batch_first = True, nonlinearity = activation_function, bidirectional = True)
        self.rnn_2 = nn.RNN(input_size = hidden_layer_dimension*2, hidden_size = hidden_layer_dimension, batch_first = True, nonlinearity = activation_function, bidirectional = True)
        self.last_linear = nn.Linear(hidden_layer_dimension*2, 4, bias = True)
    ...
    def forward(self, x, v_len):
        ...
        output_layers, final_hidden_state = self.rnn_1(x, None) #入力xと隠れ層の初期値(None)を受け取り、RNNの各時間ステップにおける隠れ層の状態と、その中でも一番最後の隠れ層の状態を出力
        output_layers, final_hidden_state = self.rnn_2(output_layers, final_hidden_state)
        temp_y = [output_layers[:, max(0, int(virtual_len)-1), :] for virtual_len in v_len]
        #y = self.softmax(self.last_linear(temp_y[0]))
...
training_step(learning_rate = 0.1, epoch_num = 500, activation_function = "tanh", batch_shuffle = True)
